...
<div x-data>
    ...
    <div x-show="$store.userModal && $store.userModal.open && $store.userModal.type !== 'delete'"
        ...
        <div class="relative bg-white rounded-xl shadow-xl w-full max-w-md p-6" @click.stop
            ...
            <div class="flex items-center justify-between mb-5">
                <h2 class="text-sm font-semibold text-zinc-900" x-text="$store.userModal.type === 'create' ? 'Add User' : 'Edit User'"></h2>
                <button @click="$store.userModal.close()" class="text-zinc-400 hover:text-zinc-700 transition-colors">
            ...
            <form @submit.prevent="$store.userModal.submit()" class="space-y-4">
                <div class="grid grid-cols-2 gap-3">
                    <div class="col-span-2 space-y-1.5">
                        ...
                        <input type="text" x-model="$store.userModal.form.name" required
                            class="w-full h-9 px-3 text-sm border border-zinc-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-zinc-900 focus:border-transparent transition">
                    </div>
                    <div class="col-span-2 space-y-1.5">
                ...
                <div class="flex gap-2 pt-1">
                    <button type="button" @click="$store.userModal.close()"
                        class="flex-1 h-9 border border-zinc-200 rounded-lg text-sm font-medium text-zinc-600 hover:bg-zinc-50 transition-colors">Cancel</button>
    ...
    <div x-show="$store.userModal && $store.userModal.open && $store.userModal.type === 'delete'"
         ...
         style="display:none">
        <div class="absolute inset-0 bg-black/40 backdrop-blur-sm" @click="$store.userModal.close()"></div>
        <div class="relative bg-white rounded-xl shadow-xl w-full max-w-sm p-6" @click.stop
             x-transition:enter="transition ease-out duration-150"
             x-transition:enter-start="opacity-0 scale-95"
             x-transition:enter-end="opacity-100 scale-100">
            <div class="w-10 h-10 rounded-full bg-red-50 flex items-center justify-center mx-auto mb-4">
            ...
            <h2 class="text-sm font-semibold text-zinc-900 text-center mb-1">Delete user?</h2>
            <p class="text-xs text-zinc-500 text-center mb-5"><strong x-text="$store.userModal.form.name"></strong> will be permanently removed.</p>
