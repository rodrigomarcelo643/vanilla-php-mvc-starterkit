<?php $prefill = Session::get('oauth_prefill') ?? []; ?>
    ...
    <div class="flex-1 flex flex-col min-h-screen">
        ...
        <div class="flex-1 flex items-center justify-center px-6 sm:px-10 py-12">
            <div class="w-full max-w-sm">
                ...
                <!-- OAuth prefill notice -->
                <?php if (!empty($prefill)): ?>
                <div class="mb-5 flex items-center gap-3 px-4 py-3 rounded-lg bg-indigo-50 dark:bg-indigo-950/40 border border-indigo-200 dark:border-indigo-800 text-sm text-indigo-700 dark:text-indigo-300">
                    <svg class="w-4 h-4 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M13 16h-1v-4h-1m1-4h.01M12 2a10 10 0 100 20A10 10 0 0012 2z"/></svg>
                    <span>Signed in with <strong><?= ucfirst(htmlspecialchars($prefill['provider'])) ?></strong>. Set a password to complete your account.</span>
                </div>
                <?php endif; ?>
