...
<div class="fade-in"
     x-data="UsersTable(<?= htmlspecialchars(json_encode($userList), ENT_QUOTES) ?>)"
     x-init="init()">
    ...
    <div class="mb-6 flex items-center justify-between gap-4">
        ...
        <button @click="$store.userModal.openCreate()"
            class="inline-flex items-center gap-2 h-9 px-4 bg-zinc-900 hover:bg-black dark:bg-white dark:text-zinc-900 dark:hover:bg-zinc-100 text-white text-sm font-medium rounded-lg transition-colors">
    ...
    <div class="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl shadow-sm overflow-hidden">
        ...
        <div id="users-skeleton" class="overflow-hidden">
                ...
                <table class="w-full text-sm">
                    ...
                    <tbody class="divide-y divide-zinc-50 dark:divide-zinc-800">
                        ...
                        <tr class="hover:bg-zinc-50 dark:hover:bg-zinc-800/50">
                            ...
                            <td class="px-4 py-3"><div class="flex items-center gap-2.5"><div class="skeleton-base w-7 h-7 rounded-full shrink-0"></div><div class="skeleton-base h-3 w-28 rounded"></div></div></td>
                            <td class="px-4 py-3"><div class="skeleton-base h-3 w-40 rounded"></div></td>
                            <td class="px-4 py-3"><div class="skeleton-base h-5 w-14 rounded-md"></div></td>
                            <td class="px-4 py-3"><div class="skeleton-base h-5 w-12 rounded-md"></div></td>
                            <td class="px-4 py-3"><div class="skeleton-base h-3 w-20 rounded"></div></td>
                            <td class="px-4 py-3"><div class="flex gap-1"><div class="skeleton-base h-7 w-14 rounded-md"></div><div class="skeleton-base h-7 w-16 rounded-md"></div></div></td>
                        </tr>
                        <?php endfor; ?>
                    </tbody>
                </table>
            ...
            <div class="flex items-center justify-between px-4 py-3 border-t border-zinc-100 dark:border-zinc-800">
                ...
                <div class="flex items-center gap-1"><?php for($i=0;$i<5;$i++): ?><div class="skeleton-base h-7 w-7 rounded-md"></div><?php endfor; ?></div>
            </div>
        </div>
                            ...
                            <th class="px-4 py-3 text-left">
                                <button @click="sort('<?= $key ?>')"
                                    ...
                                    <span class="flex flex-col leading-none">
                                        <svg class="w-2.5 h-2.5" :class="sortKey==='<?= $key ?>'&&sortDir==='asc'?'text-zinc-900 dark:text-zinc-100':'text-zinc-300'" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M5 15l7-7 7 7"/></svg>
                                        <svg class="w-2.5 h-2.5" :class="sortKey==='<?= $key ?>'&&sortDir==='desc'?'text-zinc-900 dark:text-zinc-100':'text-zinc-300'" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M19 9l-7 7-7-7"/></svg>
                                    </span>
                                </button>
                            </th>
                            <?php endforeach; ?>
                            ...
                            <tr class="hover:bg-zinc-50 dark:hover:bg-zinc-800/50 transition-colors">
                                ...
                                <td class="px-4 py-3">
                                    <div class="flex items-center gap-2.5">
                                        <div class="w-7 h-7 rounded-full bg-zinc-900 dark:bg-zinc-700 flex items-center justify-center text-white shrink-0 text-[10px] font-semibold" x-text="u.name.charAt(0).toUpperCase()"></div>
                                ...
                                <td class="px-4 py-3">
                                    <span class="inline-flex items-center px-2 py-0.5 rounded-md text-xs font-medium capitalize bg-zinc-100 dark:bg-zinc-800 text-zinc-600 dark:text-zinc-400" x-text="u.role ?? 'user'"></span>
                                </td>
                                <td class="px-4 py-3">
                ...
                <div class="flex items-center gap-1">
                    ...
                    <template x-for="p in pageRange" :key="p">
                        <button x-text="p==='...'?'…':p" @click="p!=='...'&&(page=p)"
                            :class="p===page?'bg-zinc-900 dark:bg-white dark:text-zinc-900 text-white border-zinc-900 font-semibold':'border-zinc-200 dark:border-zinc-700 text-zinc-600 hover:bg-zinc-50'"
                            :disabled="p==='...'"
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (sk && ct) setTimeout(() => { sk.style.display = 'none'; ct.style.display = ''; }, 500);
});
    ...
    return {
        ...
        init() {
            ...
            document.addEventListener('users:refresh', (e) => {
                const { res, form, deleted } = e.detail;
                if (deleted) {
                    this.rows = this.rows.filter(r => r.id !== deleted);
                    if (this.page > this.totalPages) this.page = this.totalPages;
                } else if (res?.success) {
                    if (res.id) {
                        this.rows.unshift({ id: res.id, name: form.name, email: form.email, role: form.role, status: form.status, created_at: new Date().toISOString() });
                    } else {
                        const idx = this.rows.findIndex(r => r.id === form.id);
                        if (idx !== -1) Object.assign(this.rows[idx], { name: form.name, email: form.email, role: form.role, status: form.status });
                    }
                }
            });
        },
