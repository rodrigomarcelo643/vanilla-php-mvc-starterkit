<div class="fade-in">
    ...
    <div class="mb-8">
        ...
        <p class="text-gray-500 text-sm mt-1">Welcome back, <?= htmlspecialchars(Session::get('user')['name'] ?? 'Admin') ?>!</p>
    </div>
    ...
    <div class="grid sm:grid-cols-2 xl:grid-cols-4 gap-6 mb-8">
        ...
        <div class="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 hover:shadow-md transition">
            <div class="flex items-center justify-between mb-4">
                ...
                <span class="text-xs font-semibold px-2 py-1 rounded-full <?= $s['up'] ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700' ?>">
                    <?= $s['change'] ?>
                </span>
            </div>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        <div class="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
            ...
            <a href="/?url=admin/users" class="text-sm text-indigo-600 hover:underline font-medium">View all</a>
        </div>
            ...
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-gray-50 transition">
                        <td class="px-6 py-4 font-medium text-gray-900"><?= htmlspecialchars($r['name']) ?></td>
                        <td class="px-6 py-4 text-gray-500"><?= htmlspecialchars($r['email']) ?></td>
                        <td class="px-6 py-4">
                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium <?= $r['status'] === 'active' ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-600' ?>">
                                <?= ucfirst($r['status']) ?>
                            </span>
                        </td>
