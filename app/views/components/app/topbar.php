...
<header class="h-14 bg-white border-b border-zinc-200 flex items-center px-4 shrink-0 z-20 gap-4">

    <!-- Left: hamburger + breadcrumb -->
    <div class="flex items-center gap-3 shrink-0">
        <button @click="sidebarOpen = !sidebarOpen; hovered = false"
            class="inline-flex items-center justify-center w-8 h-8 rounded-md text-zinc-500 hover:bg-zinc-100 hover:text-zinc-900 transition-colors focus:outline-none">
        ...
        <nav class="hidden sm:flex items-center gap-1.5 text-sm">
            <a href="<?= BASE_URL ?>/app/home" class="text-zinc-400 hover:text-zinc-700 transition-colors whitespace-nowrap">My Account</a>
            <svg class="w-3.5 h-3.5 text-zinc-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/>
            </svg>
            <span class="font-medium text-zinc-900"><?= $pageLabel ?></span>
        </nav>
        <span class="sm:hidden text-sm font-medium text-zinc-900"><?= $pageLabel ?></span>
    </div>
    ...
    <div class="flex-1 flex justify-center px-4" x-data="GlobalSearch([
        ...
        <div class="relative w-full max-w-md">
            ...
            <input type="text" x-model="query" @focus="open = true" @click.away="open = false"
                ...
                placeholder="Search…"
                class="w-full h-9 pl-9 pr-4 text-sm bg-zinc-50 border border-zinc-200 rounded-lg text-zinc-900 placeholder-zinc-400 focus:outline-none focus:ring-2 focus:ring-zinc-900 focus:border-transparent focus:bg-white transition">

            <div x-show="open && results.length > 0"
                 x-transition:enter="transition ease-out duration-100"
                 x-transition:enter-start="opacity-0 translate-y-1"
                 x-transition:enter-end="opacity-100 translate-y-0"
                 class="absolute top-full left-0 right-0 mt-1.5 bg-white border border-zinc-200 rounded-xl shadow-lg z-[9999] overflow-hidden"
                 style="display:none">
                <template x-for="(r, i) in results" :key="r.href">
                    <a :href="r.href" @click="open = false"
                        ...
                        <div class="w-7 h-7 rounded-md bg-zinc-100 flex items-center justify-center shrink-0">
                            <svg class="w-3.5 h-3.5 text-zinc-500" fill="none" stroke="currentColor" stroke-width="1.75" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" :d="r.icon"/>
                            </svg>
                        </div>
                        <div class="min-w-0">
                            <p class="text-sm font-medium text-zinc-900" x-text="r.label"></p>
    ...
    <div class="flex items-center gap-1 shrink-0" x-data="{ dropOpen: false, notifOpen: false }">
        ...
        <div class="relative">
            <button @click="notifOpen = !notifOpen"
                class="relative inline-flex items-center justify-center w-8 h-8 rounded-md text-zinc-500 hover:bg-zinc-100 hover:text-zinc-900 transition-colors">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
            ...
            <div x-show="notifOpen" @click.away="notifOpen = false"
                 ...
                 class="absolute right-0 mt-2 w-72 bg-white rounded-lg border border-zinc-200 shadow-lg z-50 overflow-hidden"
                 style="display:none">
                <div class="flex items-center justify-between px-4 py-3 border-b border-zinc-100">
                    ...
                    <span class="text-xs bg-zinc-100 text-zinc-600 font-medium px-1.5 py-0.5 rounded-full">2</span>
                </div>
                <?php foreach ([
                    ['Welcome to ' . APP_NAME . '!', 'just now', 'bg-green-500'],
                    ['Your profile is incomplete',   '1h ago',   'bg-yellow-500'],
                ] as $n): ?>
                <div class="flex items-start gap-3 px-4 py-3 hover:bg-zinc-50 transition-colors cursor-pointer border-b border-zinc-50 last:border-0">
                    <span class="mt-1.5 w-2 h-2 rounded-full <?= $n[2] ?> shrink-0"></span>
                    <div class="flex-1 min-w-0">
                        <p class="text-sm text-zinc-800"><?= $n[0] ?></p>
        ...
        <div class="relative">
            ...
            <div x-show="dropOpen" @click.away="dropOpen = false"
                 ...
                 x-transition:leave-start="opacity-100 scale-100"
                 x-transition:leave-end="opacity-0 scale-95"
                 class="absolute right-0 mt-2 w-52 bg-white rounded-lg border border-zinc-200 shadow-lg py-1 z-50 origin-top-right"
                 style="display:none">
                <div class="px-3 py-2 border-b border-zinc-100 mb-1">
                    <p class="text-xs font-semibold text-zinc-900 truncate"><?= htmlspecialchars($u['name'] ?? '') ?></p>
                    <p class="text-xs text-zinc-400 truncate"><?= htmlspecialchars($u['email'] ?? '') ?></p>
                </div>
                <a href="<?= BASE_URL ?>/app/profile" class="flex items-center gap-2.5 px-3 py-1.5 text-sm text-zinc-700 hover:bg-zinc-50 transition-colors">
                ...
                <a href="<?= BASE_URL ?>/app/settings" class="flex items-center gap-2.5 px-3 py-1.5 text-sm text-zinc-700 hover:bg-zinc-50 transition-colors">
                    <svg class="w-3.5 h-3.5 text-zinc-400" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                ...
                <div class="border-t border-zinc-100 mt-1 pt-1">
                    <button onclick="App.logout()" class="flex items-center gap-2.5 w-full px-3 py-1.5 text-sm text-red-500 hover:bg-red-50 transition-colors">
