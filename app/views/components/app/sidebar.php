...
<script>(function(){
    ...
    if(dk){ el.style.background='#18181b'; el.style.borderRightColor='#27272a'; }
    ...
    <div style="flex:1;padding:12px 8px;display:flex;flex-direction:column;gap:16px;overflow:hidden">
        <div>
            <div style="padding:0 8px;margin-bottom:4px">
                <div class="skeleton-base sk-label" style="height:10px;width:56px;border-radius:3px"></div>
            </div>
    ...
    <div class="border-t border-zinc-200 dark:border-zinc-800 p-2 shrink-0" x-data="{ userDrop: false }">
        ...
        <button @click="userDrop = !userDrop"
            ...
            <div x-show="sidebarOpen || hovered" class="flex-1 min-w-0" style="display:none"
                 ...
                 x-transition:leave-start="opacity-100"
                 x-transition:leave-end="opacity-0">
                <p class="text-sm font-medium text-zinc-900 dark:text-zinc-100 truncate whitespace-nowrap leading-tight"><?= htmlspecialchars($u['name'] ?? '') ?></p>
