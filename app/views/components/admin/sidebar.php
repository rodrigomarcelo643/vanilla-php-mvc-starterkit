...
$navGroups = [
    [
        'label' => 'Main',
        'items' => [
            ['uri' => 'admin/dashboard', 'label' => 'Dashboard',
...
<aside @mouseenter="if (!isMobile && !sidebarOpen) hovered = true"
    ...
    <div class="flex items-center h-14 border-b border-zinc-200 dark:border-zinc-800 shrink-0 px-3 gap-3">
        ...
        <div x-show="sidebarOpen || hovered" x-transition:enter="transition ease-out duration-150"
            ...
            <p class="text-sm font-semibold text-zinc-900 dark:text-zinc-100 truncate whitespace-nowrap"><?= APP_NAME ?>
            </p>
            ...
            <div class="space-y-0.5">
                ...
                <a href="<?= BASE_URL ?>/<?= $item['uri'] ?>" @click="if (isMobile) sidebarOpen = false"
                    ...
                    <span x-show="sidebarOpen || hovered" x-transition:enter="transition ease-out duration-150"
                        x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100"
                        x-transition:leave="transition ease-in duration-100" x-transition:leave-start="opacity-100"
                        x-transition:leave-end="opacity-0" class="flex-1 truncate whitespace-nowrap"
                        style="display:none">
                        <?= $item['label'] ?>
                    </span>
                    ...
                    <!-- Active dot -->
                    <?php if ($active): ?>
                    <span x-show="sidebarOpen || hovered" class="w-1.5 h-1.5 rounded-full bg-zinc-900 shrink-0"
                    ...
                    <div x-show="!isMobile && !sidebarOpen && !hovered"
                        ...
                        style="display:none">
                        <div
                            class="bg-zinc-900 text-white text-xs font-medium px-2.5 py-1.5 rounded-md shadow-lg whitespace-nowrap">
                            <?= $item['label'] ?>
                        </div>
                    </div>
                </a>
    ...
    <div class="border-t border-zinc-200 dark:border-zinc-800 p-2 shrink-0" x-data="{ userDrop: false }">
        ...
        <button @click="userDrop = !userDrop"
            ...
            <div class="w-7 h-7 rounded-full bg-zinc-900 flex items-center justify-center text-white shrink-0 font-semibold overflow-hidden"
            ...
            <div x-show="sidebarOpen || hovered" x-transition:enter="transition ease-out duration-150"
                ...
                x-transition:leave="transition ease-in duration-100" x-transition:leave-start="opacity-100"
                x-transition:leave-end="opacity-0" class="flex-1 min-w-0" style="display:none">
                <p class="text-sm font-medium text-zinc-900 dark:text-zinc-100 truncate whitespace-nowrap leading-tight"
                    data-user-name><?= htmlspecialchars($u['name'] ?? 'Admin') ?></p>
                <p class="text-xs text-zinc-400 truncate whitespace-nowrap leading-tight" data-user-email>
                    <?= htmlspecialchars($u['email'] ?? '') ?></p>
            </div>

            <!-- Chevron — shown when expanded -->
            <svg x-show="sidebarOpen || hovered" class="w-3.5 h-3.5 text-zinc-400 shrink-0 ml-auto" fill="none"
