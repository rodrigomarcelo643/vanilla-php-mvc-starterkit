...
### Fixed
- **ErrorController:**
  - Added comprehensive dynamic content negotiation to return structured JSON responses for AJAX/API requests instead of HTML templates.
  - Resolved missing controller methods (`internalError`, `forbidden`, `notImplemented`, `serviceUnavailable`, `maintenance`) in Option 3 (Backend Only) preset, preventing PHP Fatal Errors during unhandled exceptions.
